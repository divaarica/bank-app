...
<main class="content">
    <div class="container-fluid p-0">
        <h1 class="h3 mb-3"><strong>Type de compte</strong></h1>
        @if(session('success'))
        ...
        @enderror
        <a href="" type="button" class="btn btn-primary float-end padding-right-10px" data-toggle="modal" data-target="#addClasseModal"><i class="fa fa-plus"></i> Ajouter</a><br>
        <br>
            ...
            <table id="myTable" class="table table-striped table-light">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$t -> libelle }}</td>
                        <td class="d-flex ">
                            <a href="" type="button" class="btn btn-success custom-btn" data-toggle="modal" data-target="#editType{{$t->id}}">
                                Modifier</a>&nbsp;&nbsp;&nbsp;

                            <form action="{{ route('categories.destroyTypeCompte', $t -> id)}} " method="post" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ?');" style="display:inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addClasseModalLabel">Ajouter une nouveau type de compte</h5>
                    <button type="button" class="close" aria-label="Fermer" data-dismiss="modal">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('categories.createTypeCompte') }}" method="post">
                        @csrf
                        @method('PUT')
    ...
    <!-- modal modifier -->
    @foreach( $types as $t)
    <div class="modal fade" id="editType{{$t->id}}" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Modifier le Type de comtee</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Fermer">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('categories.updateTypeCompte', ['type' => $t->id]) }}" method="post">
                        @csrf
                        @method('PATCH')

                        <div class="form-group">
                            <label for="example-text-input" class="form-control-label">Libele</label>
                            <br>

                            <input class="form-control" id="libelle" name="libelle" type="text" value="{{ $t -> libelle  }}">
                        </div>
                        <br>
